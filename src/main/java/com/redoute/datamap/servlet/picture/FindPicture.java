...
public class FindPicture extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Picture picture = datamapService.findPictureByKey(name);

            JSONObject jsonResponse = new JSONObject();

            JSONArray row = new JSONArray();
            row.put(picture.getId())
                    .put(picture.getPage())
                    .put(picture.getPicture())
                    .put(picture.getBase64());

            jsonResponse.put("picture", row);

            response.setContentType("application/json");
            response.getWriter().print(jsonResponse.toString());
        } catch (JSONException ex) {
            com.redoute.datamap.log.Logger.log(FindAllPicture.class.getName(), org.apache.log4j.Level.FATAL, ex.toString());
        } finally {
            out.close();
        }
    }
